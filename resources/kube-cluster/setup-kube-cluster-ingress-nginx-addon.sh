...
#curl -s -L https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v0.46.0/deploy/static/provider/baremetal/deploy.yaml \
#        | sed 's/targetPort: http$/&\n      nodePort: 31080/' \
#        | sed 's/targetPort: https$/&\n      nodePort: 31443/' \
#		> /etc/kube-cluster/ingress-nginx-nodeport-deploy.yaml

curl -s -L https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.2.0/deploy/static/provider/baremetal/deploy.yaml \
        | sed 's/targetPort: http$/&\n    nodePort: 31080/' \
        | sed 's/targetPort: https$/&\n    nodePort: 31443/' \
		> /etc/kube-cluster/ingress-nginx-nodeport-deploy.yaml
